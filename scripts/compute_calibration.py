...
try:
	(imu_trans,imu_rot) = listener.lookupTransform('base_link','imu_link',rospy.Time(0))
...
for topic, msg, time in bag.read_messages(topics=("/imu/rpy", "imu/rpy")):	
	quat = quaternion_from_euler(msg.vector.x, msg.vector.y, msg.vector.z)
	transformed_msg = euler_from_quaternion(quat*imu_rot)
	time_yaw_tuples.append((time.to_sec(), transformed_msg[2]))
...
for topic, msg, time in bag.read_messages(topics=("/imu/mag", "imu/mag")):
  if time.to_sec() > time_start and time.to_sec() < time_end:
	quat = quaternion_from_euler(msg.vector.x, msg.vector.y, msg.vector.z)
	t_msg = euler_from_quaternion(quat*imu_rot)
	vecs.append((t_msg[0], t_msg[1], t_msg[2]))
